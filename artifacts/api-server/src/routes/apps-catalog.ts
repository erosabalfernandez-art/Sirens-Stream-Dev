import { Router } from 'express';
  ...
  router.post('/apps-catalog', async (req, res) => {
    ...
    try {
      ...
      // 2. Auto-provision channel_requests for all users who already have approved channels
      //    (fire-and-forget — don't block the response)
      setImmediate(async () => {
        try {
          // Find all distinct user_ids with any approved channel_request
          const existingRes = await fetch(
            sbUrl('channel_requests?status=eq.approved&select=user_id'),
            { headers: sbH() }
          );
          if (!existingRes.ok) return;
          const existing: { user_id: string }[] = await existingRes.json();
          const uniqueUsers = [...new Set(existing.map(e => e.user_id))];
          if (uniqueUsers.length === 0) return;

          // Check which users already have a channel_request for this new app
          const alreadyRes = await fetch(
            sbUrl(`channel_requests?app_name=eq.${encodeURIComponent(name)}&select=user_id`),
            { headers: sbH() }
          );
          const already: { user_id: string }[] = alreadyRes.ok ? await alreadyRes.json() : [];
          const alreadySet = new Set(already.map(e => e.user_id));

          // Insert approved channel_requests for users who don't have one yet
          const toInsert = uniqueUsers.filter(uid => !alreadySet.has(uid));
          ...
          req.log?.info({ app: name, provisioned: toInsert.length }, 'channel_requests auto-provisioned for new app');
        } catch (e) {
          req.log?.warn({ err: e }, 'channel auto-provisioning failed (non-critical)');
        }
      });
